...
void Grid::repositioning() {
  ...
  for (auto& block: blocks_) {
    for (auto [id, position, hv, velocity, _, _]: block.particles()) {
      aux[getBlockIndex(position)].addParticle(id, position, hv, velocity);
    }
  }
...
u32 Grid::getBlockIndex(math::vec3 const& particle_pos) const {
  ...
  return (
      static_cast<u32>(pos_i) + (static_cast<u32>(pos_j) * grid_size_.x) +
 ...
 * @return
 */
bool Grid::blockInBounds(math::Vec3<int> const& block_pos) const {
  return block_pos.x >= 0 && static_cast<u64>(block_pos.x) < grid_size_.x && block_pos.y >= 0 &&
         static_cast<u64>(block_pos.y) < grid_size_.y && block_pos.z >= 0 &&
         static_cast<u64>(block_pos.z) < grid_size_.z;
}

void Grid::addBlockToLimits(u32 const index, math::Vec3<i32> const& neighbor_pos) {
  if (neighbor_pos.x < 0) {
